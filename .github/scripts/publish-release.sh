...
# dots), so "Keypass Setup 1.0.9.exe" is stored as "Keypass.Setup.1.0.9.exe".
...
delete_asset_id() {
  ...
  local status
  status="$(
    curl -sS -o /tmp/keypass-delete-asset.json -w "%{http_code}" \
      -X DELETE \
  ...
  case "$status" in
    ...
    *)
      ...
      cat /tmp/keypass-delete-asset.json >&2 || true
      echo >&2
      return 1
      ;;
  esac
...
upload_assets() {
  ...
  for file in "$DIST"/*.dmg "$DIST"/*.blockmap "$DIST"/*.yml; do
    upload_one "$id" "$file"
  done
  log_assets "$id"
}
...
stage_artifacts() {
  ...
  while IFS= read -r -d '' file; do
    cp "$file" "$DIST/"
    found=1
  done < <(find artifacts -type f \( -name '*.dmg' -o -name '*.blockmap' -o -name 'latest*.yml' \) -print0)
